...
void NewProcISR(func_p_t p) {  // arg: where process code starts
   ...
   if( avail_q.bytes == 0 ) {    // may occur if too many been created
...
void TimerISR(void) {
   ...
   pcb[cur_pid].cpu_time++;              // count up time
   pcb[cur_pid].cpu_life++;             // count up life

   if(pcb[cur_pid].cpu_time == TIME_MAX) {       // if runs long enough
